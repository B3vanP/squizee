...
# Function to load questions (now redundant since questions are embedded)
def load_questions():
    return questions_data

# Function to quiz the user
def quiz_user(questions, num_questions):
    score = 0
    missed_questions = []
    selected_questions = questions[:num_questions]  # Select the requested number of questions
    ...
    for i, question in enumerate(selected_questions):
        ...
        while True:
            try:
                answer = input("Your answer(s) (1-6, separate multiple answers with commas): ").split(',')
                answer = [int(a.strip()) for a in answer]
                # Check if the inputs are within the valid range
                if all(1 <= a <= len(question['options']) for a in answer):
                    break
                else:
                    print(f"Please enter numbers between 1 and {len(question['options'])}.")
            except ValueError:
                print(f"Invalid input. Please enter numbers between 1 and {len(question['options'])}.")

        correct_answers = [question['options'].index(ans) + 1 for ans in question['answer']] if isinstance(question['answer'], list) else [question['options'].index(question['answer']) + 1]
        ...
        if sorted(answer) == sorted(correct_answers):
            ...
            correct_ans_words = ', '.join([question['options'][a - 1] for a in correct_answers])
            print(f"Wrong! The correct answer(s) is/are: {correct_ans_words}\n")
    
    # Print the final score
    print(f"Your final score is {score} out of {num_questions}")
    return missed_questions, selected_questions

# Function to handle the quiz process
def handle_quiz(questions):
    all_questions = questions[:]
    answered_questions = []
    missed_questions = []

    while True:
        # Ask the user how many questions they would like in the quiz
        while True:
            try:
                num_questions = int(input("How many questions would you like in the quiz? (Max 100): "))
        ...
        # Select the questions for this quiz session
        available_questions = [q for q in all_questions if q not in answered_questions]
        if len(available_questions) < num_questions:
            num_questions = len(available_questions)
            print(f"Only {num_questions} questions left to answer.")

        random.shuffle(available_questions)
        quiz_questions = available_questions[:num_questions]

        # Start the quiz with the selected questions
        missed, selected = quiz_user(quiz_questions, num_questions)
        missed_questions.extend(missed)
        answered_questions.extend(selected)

        while True:
            print("\nOptions:")
            print("1. Answer more questions.")
            print("2. Retry the questions you missed.")
            print("3. Exit.")
            
            try:
                option = int(input("Please select an option (1-3): "))
                if option == 1:
                    break  # Continue to the outer while loop to start a new quiz
                elif option == 2:
                    if missed_questions:
                        questions = missed_questions
                        random.shuffle(questions)
                        missed_questions, _ = quiz_user(questions, len(questions))
                    else:
                        print("No missed questions to retry.")
                elif option == 3:
                    print("Goodbye!")
                    return
                else:
...
# Main function
def main():
    # Load the questions (now directly from the embedded data)
    questions = load_questions()
    # Handle the quiz process
    handle_quiz(questions)
